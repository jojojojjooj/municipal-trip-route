...
출장 설계 화면의 `캘린더` 버튼은 출장일을 종일 일정으로 담은 표준 iCalendar(`.ics`) 파일을 생성합니다. 출장명·담당자·부서·출발지·왕복 여부·예상 거리·소요 시간·방문 순서·현장 메모가 일정 설명에 포함되므로 모바일 캘린더, Google Calendar, Outlook, Apple 캘린더 등에서 열어 현장 일정으로 관리할 수 있습니다. 이 기능은 외부 캘린더 계정에 자동으로 등록하지 않고 파일만 생성하므로, 사용자가 원하는 캘린더를 직접 선택할 수 있습니다.
