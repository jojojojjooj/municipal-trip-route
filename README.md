...
| 우선순위 | 기능                            | 기대 효과                                           | 구현 방향                                                                   |
| -------- | ------------------------------- | --------------------------------------------------- | --------------------------------------------------------------------------- |
| 1        | 현장용 오프라인 PWA와 동기화 큐 | 통신이 불안정한 현장에서도 상태·메모·사진 기록 보존 | 서비스 워커, IndexedDB outbox, 재연결 후 충돌 해결 정책 도입                |
| 2        | 감사 로그·변경 이력             | 공공 업무의 책임 추적과 보고 근거 강화              | 계획·목적지·권한 변경 이벤트를 append-only 로그로 저장하고 타임라인 UI 제공 |
| 3        | 오프라인 사진 업로드 재시도 큐  | 현장 통신 불안정 시 사진 기록 유실 방지             | IndexedDB outbox와 업로드 재시도 상태를 현장 기록에 연결                    |
| 4        | 실시간 협업·알림                | 담당자 배정과 이슈 조치 지연 감소                   | 이슈 담당자 알림, 변경 구독, 미해결 기한 알림을 서버 작업으로 분리          |
| 5        | 차량·출장비·결과 통계           | 출장 운영을 동선 관리에서 비용·성과 관리로 확장     | 차량/비용 테이블과 월별 비용·거리·완료율 분석 추가                          |
