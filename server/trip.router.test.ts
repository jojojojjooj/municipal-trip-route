  ...
  stops: [
    {
      ...
      photos: [
        {
          storageKey: "trip-photos/7/site.jpg",
          url: "/manus-storage/trip-photos/7/site.jpg",
          fileName: "현장.jpg",
          takenAt: "2026-08-21",
          description: "민원실 출입 동선과 현장 안내 표지판을 확인한 사진",
        },
      ],
    },
  ],
...
describe("trip router", () => {
  ...
  it("creates a user-owned trip with a generated share token", async () => {
    dbMocks.createTrip.mockResolvedValue({
      id: 11,
      ...input,
    ...
    expect(dbMocks.createTrip).toHaveBeenCalledWith(
      7,
  ...
  it("rejects duplicate stop sequences before persisting a trip", async () => {
    const duplicateSequenceInput = {
      ...input,
      stops: [
        ...input.stops,
        { ...input.stops[0], name: "중복 순서 목적지", sequence: 1 },
      ],
    };
    await expect(
      appRouter.createCaller(context()).trip.create(duplicateSequenceInput)
    ).rejects.toMatchObject({ code: "BAD_REQUEST" });
    expect(dbMocks.createTrip).not.toHaveBeenCalled();
  });
  ...
  it("marks a saved trip as a repeatable template through the owner-only procedure", async () => {
    ...
    await expect(
      appRouter
        ...
        .trip.template.toggle({ tripId: 11, isTemplate: true })
    ).resolves.toEqual({ success: true, isTemplate: true });
    expect(dbMocks.updateTripTemplateForOwner).toHaveBeenCalledWith(
      7,
      11,
      true
    );
  ...
  it("validates batch dates before creating trips from a template", async () => {
    dbMocks.createTripsFromTemplateForOwner.mockResolvedValue({
      status: "created",
      trips: [
        { id: 21, title: "정기 점검 · 2026-09-01", tripDate: "2026-09-01" },
      ],
    });
    const caller = appRouter.createCaller(context());

    await expect(
      caller.trip.template.createBatch({
        templateId: 11,
        dates: ["2026-09-12", "2026-09-01"],
        titlePrefix: "정기 점검",
        managerName: "담당자",
      })
    ).resolves.toMatchObject({ status: "created" });
    expect(dbMocks.createTripsFromTemplateForOwner).toHaveBeenCalledWith(
      7,
      11,
      expect.objectContaining({
  ...
  it("lists, deletes, and reads shared trip data through ownership-aware helpers", async () => {
    dbMocks.listTripsForUser.mockResolvedValue([
      { id: 11, title: input.title, access: "owner" },
    ]);
    dbMocks.deleteTripForOwner.mockResolvedValue(true);
    dbMocks.getSharedTrip.mockResolvedValue({
      id: 11,
      ...input,
      shareToken: "share-token",
      stops: input.stops,
    ...
    await expect(caller.trip.remove({ id: 11 })).resolves.toEqual({
      success: true,
    });
    await expect(
      caller.trip.shared({ token: "share-token" })
    ).resolves.toMatchObject({ id: 11, title: input.title });
  ...
  it("stores, reads, and clears only the authenticated user's temporary draft", async () => {
    ...
    dbMocks.upsertTripDraftForOwner.mockResolvedValue({
      ownerId: 7,
      payload,
      updatedAt: new Date(),
    });
    dbMocks.getTripDraftForOwner.mockResolvedValue({
      ownerId: 7,
      payload,
      updatedAt: new Date(),
    });
    ...
    await expect(caller.trip.draft.save({ payload })).resolves.toMatchObject({
      ownerId: 7,
      payload,
    });
    await expect(caller.trip.draft.get()).resolves.toMatchObject({
      ownerId: 7,
      payload,
    });
  ...
  it("updates execution status and checklist through authenticated editor-capable helpers", async () => {
    ...
    await expect(
      caller.trip.updateStopExecution({
        stopId: 91,
        executionStatus: "completed",
        completedAt: "2026-08-22T01:10:00.000Z",
        issueNote: "배수로 확인",
        issueOwner: "현장반",
        issueDueAt: "2026-08-23",
        issueResolvedAt: "2026-08-22T01:10:00.000Z",
      })
    ).resolves.toEqual({ success: true });
    await expect(
      caller.trip.updateChecklist({ tripId: 11, checklist: input.checklist })
    ).resolves.toEqual({ success: true });

    expect(dbMocks.updateTripStopExecutionForUser).toHaveBeenCalledWith(7, 91, {
      executionStatus: "completed",
      completedAt: "2026-08-22T01:10:00.000Z",
      issueNote: "배수로 확인",
      issueOwner: "현장반",
    ...
    expect(dbMocks.updateTripChecklistForUser).toHaveBeenCalledWith(
      7,
      11,
      input.checklist
    );
  ...
  it("stores an editable result report draft through the editor-capable helper", async () => {
    ...
    const draft = {
      ...
      evidenceOrder: ["photo-3", "photo-1"],
      excludedEvidenceKeys: ["photo-2"],
    };

    await expect(
      appRouter
        .createCaller(context())
        .trip.updateReportDraft({ tripId: 11, draft })
    ).resolves.toEqual({ success: true });
    expect(dbMocks.updateTripReportDraftForUser).toHaveBeenCalledWith(
      7,
      11,
  ...
  it("lets only the owner-facing helper manage collaborators and department", async () => {
    ...
    dbMocks.inviteTripCollaboratorForOwner.mockResolvedValue({
      status: "invited",
    });
    dbMocks.listTripCollaboratorsForOwner.mockResolvedValue([
      {
        id: 8,
        userId: 9,
        name: "협업자",
    ...
    await expect(
      caller.trip.updateDepartment({ tripId: 11, department: "안전총괄과" })
    ).resolves.toEqual({ success: true });
    await expect(
      caller.trip.collaborators.invite({
        tripId: 11,
        email: "collab@example.com",
        permission: "editor",
      })
    ...
    await expect(
      caller.trip.collaborators.list({ tripId: 11 })
    ).resolves.toHaveLength(1);
    await expect(
      caller.trip.collaborators.remove({ tripId: 11, collaboratorId: 8 })
    ).resolves.toEqual({ success: true });
    expect(dbMocks.updateTripDepartmentForOwner).toHaveBeenCalledWith(
      7,
      11,
      "안전총괄과"
    );
    expect(dbMocks.inviteTripCollaboratorForOwner).toHaveBeenCalledWith(
      7,
      11,
      "collab@example.com",
      "editor"
  ...
  it("returns the authenticated user's actual six-month analytics scope", async () => {
    dbMocks.getTripAnalyticsForUser.mockResolvedValue({
      totalTrips: 2,
      totalStops: 5,
      completedStops: 3,
      completionRate: 60,
    ...
    await expect(
      appRouter.createCaller(context()).trip.analytics()
    ).resolves.toMatchObject({ completionRate: 60, openIssues: 1 });
  ...
  it("uses address search before keyword fallback and reverse-geocodes map clicks", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({
        ...
        json: async () => ({
          documents: [
            {
              address: { address_name: "전북특별자치도 정읍시" },
              x: "126.86",
              y: "35.57",
            },
          ],
    ...
    await expect(
      caller.trip.searchAddress({ query: "정읍시청" })
    ).resolves.toMatchObject([{ name: "정읍시청" }]);
    await expect(
      caller.trip.reverseGeocode({ latitude: 35.57, longitude: 126.86 })
    ).resolves.toEqual({ address: "전북특별자치도 정읍시" });
  ...
  it("falls back to formatted coordinates when reverse geocoding has no address", async () => {
    ...
    const result = await appRouter
      .createCaller(context())
      .trip.reverseGeocode({ latitude: 37.5665, longitude: 126.978 });
